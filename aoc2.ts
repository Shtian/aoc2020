...
const mappedInput = input.map(i => {
    const [,min, max, letter, password] = i.trim().match(regex);
    return {min, max, letter, password};
});

/* part 1 */
const validPasswords = mappedInput.filter(({min, max, letter, password}) => {
    if(password.length < min) return false;
    let numberOfLetters = password.split('').filter(s => s === letter).length;
    return numberOfLetters >= min && numberOfLetters <=max
}); //416

/* part 2 */
const validPasswords2 = mappedInput.filter(({min, max, letter, password}) => ((password[min-1] === letter) !== (password[max-1] === letter))); //688
