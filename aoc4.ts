...
const allPassports: passport[] = input.map((i) =>
    i.map(([ k, v ]) => ({ k, v })).reduce((passport, field) => ({ ...passport, [field.k]: field.v }), {})
);
...
const validateHasAllRequiredFields = (p) => requiredFields.reduce((a, b) => a && !!p[b], true);

const createNumberValidator = (min, max) => (i) => Number(i) <= max && Number(i) >= min;
const createRegexValidator = (regex: RegExp) => (str) => regex.test(str);

const isYear = createRegexValidator(/^[\d]{4}$/);
const createYearValidator = (from, to) => (year) => isYear(year) && createNumberValidator(from, to)(year);

const heightRegExp = new RegExp(/^([\d]{2,3})(cm|in)$/);
const isHeight = createRegexValidator(heightRegExp);
const validCmHeight = createNumberValidator(150, 193);
const validInHeight = createNumberValidator(59, 76);
const getHeight = (height: string) => height.match(heightRegExp);
const heightValid = ([ , value, unit ]: RegExpMatchArray) =>
    unit === 'cm' ? validCmHeight(value) : validInHeight(value);

/* Part 1 */
const answer4a = allPassports.filter(validateHasAllRequiredFields).length;

/* Part 2 */
const validateByr = (p) => createYearValidator(1920, 2002)(p[allFields.byr]);
const validateIyr = (p) => createYearValidator(2010, 2020)(p[allFields.iyr]);
const validateEyr = (p) => createYearValidator(2020, 2030)(p[allFields.eyr]);
const validateHcl = (p) => createRegexValidator(/^#[a-f0-9]{6}$/)(p[allFields.hcl]);
const validateEcl = (p) => [ 'amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth' ].includes(p[allFields.ecl]);
const validatePid = (p) => createRegexValidator(/^[\d]{9}$/)(p[allFields.pid]);
const validateHgt = (p) => isHeight(p[allFields.hgt]) && heightValid(getHeight(p[allFields.hgt]));

const answer4b = allPassports
    .filter(validateHasAllRequiredFields)
    .filter(validateByr)
    .filter(validateIyr)
    .filter(validateEyr)
    .filter(validateHgt)
