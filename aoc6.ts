...
wdfkpmalijbncuvr
qhnmikpzaygxwsovej`.split('\n\n');

const answer6a = input6.map((i) => i.split('\n').join('')).reduce((count, group) => {
    const charSet = new Set<string>();
    group.split('').forEach((s) => charSet.add(s));
    return (count += charSet.size);
}, 0);

const answer6b = input6.map((i) => i.split('\n')).reduce((count, group) => {
    const charSet = {};
    group.join('').split('').forEach((s) => (charSet[s] ? charSet[s]++ : (charSet[s] = 1)));
    return (count += Object.keys(charSet).filter((key) => charSet[key] >= group.length).length);
}, 0);
